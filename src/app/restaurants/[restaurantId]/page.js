...
export default async function RestaurantPage({ params }) {
  ...
  return (
    <Layout>
      <div className="mb-12">
        <h1 className="text-4xl font-bold mb-6">Available Foods</h1>
        <div className="bg-green-100 border border-green-300 rounded-lg p-4 mb-8">
          <h2 className="font-semibold text-green-800 mb-2">📍 Available Now</h2>
          <p className="text-green-700">No ID required • First come, first served</p>
        </div>
      </div>
        ...
        {restaurantFoods.map((food) => (
          <Card key={food.id} title={food.name}>
            <div className="space-y-2 text-sm">
              ...
              <p><span className="font-medium">Notes:</span> {food.notes}</p>
              <p className="text-orange-600 font-semibold">Available until {food.availableUntil}</p>
            </div>
          </Card>
        ))}
      </div>
